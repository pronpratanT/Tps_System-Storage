...
function ChartImport({ refresh, shouldRefresh }) {
    ...
    async function setupChart() {
      try {
        ...
        importData.forEach((item) => {
          const date = new Date(item.dateImport);
          const year = date.getFullYear();
          const month = date.getMonth();

          if (year === currentYear || year === lastYear) {
            item.selectedProduct.forEach((product) => {
              monthlyData[year][month] += parseInt(product.import) || 0;
            });
          }
        });
      ...
      <div className="px-4 py-3 bg-transparent">
        <div className="flex items-center">
          <div className="flex-1">
            ...
            <h2 className="text-xl font-semibold text-gray-700">Import Statistics</h2>
          </div>
        </div>
      </div>
      <div className="p-4">
